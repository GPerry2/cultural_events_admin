                ...
                "rows":[
                  ...
                  },
                  {
                    "fields": [
                      ...
                      {"id": "venue_lookup","type":"button","title":"Verify","addclass":"vlookup","onclick":function(e){}}
                    ]
                  },
                  {
                    "fields":[
      ...
      section = [
        ...
        },
        {
          ...
          "rows": [
            ...
            {
              "fields": [
                {"id": "he_unitNumber", "bindTo": "he_unitNumber", "title": "Unit Number", "required": false},
...
function generateBabiesFile(showDone){
  ...
  $.ajax({
    ...
    let configName = "dc_ce_hotspot.json";

    putConfigWithData(payload, configURL, appName, configName, function () {});
...
function generateMediaFile(showDone){
...
function patchConfig(dataURL, configURL, appName, configName, targetObject, successCallBack, processor) {
  ...
  $.when(
    ...
    .done(function (first, second) {
      ...
      updateConfig(configURL, "system_variables.js", "application/javascript", "var sys_config = " + payload + "", getCookies(config.default_repo + ".sid")).then(function () {

      });
      updateConfig(configURL, "site_variables.json", "application/json", payload, getCookie(config.default_repo + ".sid")).then(
        function () {
...
function patchConfigWithData(conf, configURL, appName, configName, targetObject, successCallBack, processor) {
  let deferred = new $.Deferred();
  ...
  */
  ...
  $.ajax(
    ...
    let payload = JSON.stringify($.extend(data, conf));
    updateConfig(configURL, configName, "application/json", payload, getCookie(config.default_repo + ".sid")).then(
      function () {
        if (typeof successCallBack === "function") {
          successCallBack(conf)
        }
...
function putConfigWithData(conf, configURL, appName, configName, targetObject, successCallBack, processor) {
  let deferred = new $.Deferred();
...
function updateConfig(entitySet, name, contentType, media, sid) {
